\section{Gauss- und Gauss-Jordan-Algorithmus}
...
\begin{itemize}
    \item $\text{rg}(A) < \text{rg}(A|\vec{b})$: Keine Lösung (inkonsistent)
    \item $\text{rg}(A) = \text{rg}(A|\vec{b}) = n$: Eindeutige Lösung
    \item $\text{rg}(A) = \text{rg}(A|\vec{b}) < n$: Unendlich viele Lösungen, Parameter: $n - \text{rg}(A)$
\end{itemize}
