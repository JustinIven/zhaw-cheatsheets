...
Der Übergang \(\delta(q_0, a) = q_1\) wird grafisch dargestellt als\\
\begin{center}
    \begin{tikzpicture}[main/.style = {draw, circle}]
        \node[main] (q) {$q_0$};
        \node[main] (p) [right=1.5cm of q] {$q_1$};
        \draw[->] (q) to node[above] {\(a\)} (p);
    \end{tikzpicture}
\end{center}
...
\textbf{Akzeptanz}: Es existiert mindestens ein Lauf, der in einem Zustand aus \(F\) endet.

\subsection{Äquivalenz von DEA und NEA}
Ein NEA \(N=(Q_N,\Sigma,\delta_N,q_{0N},F_N)\) kann in einen equivalenten DEA \(D=(Q_D,\Sigma,\delta_D,q_{0D},F_D)\) überführt werden durch
\begin{itemize}
