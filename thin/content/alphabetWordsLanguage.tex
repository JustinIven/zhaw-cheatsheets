...
\subsection{Alphabet}
Ein Alphabet ist eine endliche, nichtleere Menge von Symbolen. Das  Bsp.:
\[
    \Sigma = \{a,b,c\}, \quad
...
\subsubsection{\texorpdfstring{Wörter der Länge \(k\)}{Wörter der Länge k}}
...
Die \(n\)-te Potenz eines Wortes \(x\) wird für alle \(n \in \mathbb{N}\) rekursiv definiert als
\begin{align*}
    x^0 &:= \varepsilon \\
    x^{n+1} &:= x^n x
\end{align*}
...
\begin{itemize}
    ...
    \item \(\varepsilon\) ist nicht allgemein in jeder Sprache \(L\) enthalten.
\end{itemize}
